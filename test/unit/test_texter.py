...
class TexterTestCase(unittest.TestCase):
    ...
    def testDec(self):
        ...
        assert [t.label for t in ticks] == ["0.000,000,1", "0.000,000,01", "0.000,000,001"]
        ...
        assert [t.label for t in ticks] == ["1,000,000", "10,000,000", "100,000,000"]

    def testExp(self):
        ticks = [tick((-1, 10), labellevel=0), tick((1, 1), labellevel=0), tick((10, 1), labellevel=0)]
        exponentialtexter().labels(ticks)
        assert [t.label for t in ticks] == [r"{-10^{-1}}", r"{10^{0}}", r"{10^{1}}"]
        ...
        assert [t.label for t in ticks] == [r"{0.0}", r"{{-1.0}\cdot10^{-1}}", r"{{1.5}\cdot10^{-1}}"]
    ...
    def testDefault(self):
        ...
        assert [t.label for t in ticks] == ["0", "0.1", "1", "10"]
        ticks = [tick((0, 10), labellevel=0), tick((1, 10), labellevel=0), tick((1, 1), labellevel=0), tick((10000, 1), labellevel=0)]
        defaulttexter().labels(ticks)
        assert [t.label for t in ticks] == [r"{0}", r"{{1}\cdot10^{-1}}", r"{1}", r"{{1}\cdot10^{4}}"]
        ticks = [tick((0, 10), labellevel=0), tick((1, 10), labellevel=0), tick((1, 1), labellevel=0), tick((10000, 1), labellevel=0)]
        defaulttexter(equaldecision=0).labels(ticks)
        assert [t.label for t in ticks] == ["0", "0.1", "1", r"{10^{4}}"]
