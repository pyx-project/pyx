...
def hpaint(c, x, y, axis, reverse = 0):
    ...
    axis.ticks = axis.part.defaultpart(axis.min/axis.factor, axis.max/axis.factor, 0, 0)
...
vpaint(c, 11, 0, graph.linaxis(painter=graph.axispainter(tickattrs=color.rgb.red, innerticklength=0, outerticklength=0.2), **lintest))
try:
    vpaint(c, 11, 0, graph.linaxis(painter=graph.axispainter(tickattrs=((color.rgb.red,),)), **lintest), reverse=1)
    raise Exception("IndexError should have been raised.")
except IndexError: pass
vpaint(c, 12, 0, graph.linaxis(painter=graph.axispainter(tickattrs=((color.rgb.red,), (color.rgb.green,)), subticklengthfactor=2), **lintest), reverse=1)
