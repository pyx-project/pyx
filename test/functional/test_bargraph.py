...
def test_bar4(c, x, y):
    g = c.insert(graph.graphxy(x, y, height=5, width=12, x=graph.axis.nestedbar(graph.axis.bar(dist=0, painter=graph.axis.painter.bar(basepathattrs=None))), key=graph.key.key()))
    g.plot([graph.data.file("data/testdata2", xname=0, y=2, ystack1=3, title="test"),
            graph.data.file("data/testdata2", xname=0, y="$2-1", ystack1=3, every=2)],
