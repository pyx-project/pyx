...
The class \verb|mark| can be used to plot symbols, errorbars and
lines configurable by parameters of the constructor. Providing
\verb|None| to attributes hides the according component.

\medskip
\begin{tabularx}{\linewidth}{ll>{\raggedright\arraybackslash}X}
argument name&default&description\\
\hline
\texttt{mark}&\texttt{changemark.cross()}&symbol to be used\\
\texttt{size}&\texttt{"0.2 cm"}&size of the symbol (visual length)\\
\texttt{markattrs}&\texttt{canvas.stroked()}&draw attributes for the symbol\\
...
The class \verb|line| is a inherited and constricted \verb|mark|. The
constructor takes only \verb|lineattrs| and its default is set to
\verb|changelinestyle()|.

\subsection{Rectangular marks}

\subsection{Text marks}

\subsection{Arrow marks}
