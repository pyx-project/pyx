...
\begin{classdesc}{bar}{barattrs=[], epsilon=1e-10, gradient=color.gradient.RedBlack} % {{{
  ...
  On 3d graphs \var{epsilon} is used to prevent numerical
  instabilities on bars without hight. When \var{gradient} is not
  \code{None} it is used to calculate a lighting coloring taking into
  account the angle between the view ray and the bar and the distance
  between viewer and bar. The precise conversion is defined in the
