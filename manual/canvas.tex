...
\begin{methoddesc}{pipeGS}{filename="-", device=None, resolution=100,
  ...
  ends in \texttt{.png} and \code{"jpeg"} is used when the filename
  ends in \texttt{.jpg}.
  ...
  \var{ciecolor} adds the \texttt{-dUseCIEColor} flag to improve
  the CMYK to RGB color conversion. \var{input} can be either
  \code{"eps"} or \code{"pdf"} to select the input type to be passed
