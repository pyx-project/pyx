...
\begin{tabularx}{\linewidth}{>{\hsize=.7\hsize}X>{\raggedright\arraybackslash\hsize=1.3\hsize}X}
  ...
  \texttt{t}.$^\dagger$\\
  \texttt{lentopar(l, \newline\phantom{lentopar(}epsilon=1e-5)} & returns the parameter value corresponding to the
  lengths \texttt{l} (one or a list of lengths). This uses
  arclength-calculations with accuracy \texttt{epsilon}.$^\dagger$\\
