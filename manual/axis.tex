...
\begin{classdesc}{bar}{subaxes=None,
  ...
  coordinates. For that, the axis gets a tuple of two values. The
  first item is taken to be one of the discrete values valid on this
  axis. The second item is passed to the corresponding subaxis. The
  result of the conversion done by the subaxis is mapped to the graph
  ...
  \var{subaxes} should be a list to map a discrete value of the bar
  axis to the corresponding subaxis. Since the subaxis have to have an
  order, the discrete values of the bar axis need to be integers
  starting at 0 for that case. When no subaxes are set or data is
  recieved for a unknown descrete axis value, instances of
  defaultsubaxis are used as the subaxis for this discrete value. For
  this case, the discrete values can be any hashable type and the
  order of the subaxis is defined by the order the data is recieved
  or reversed when \var{reverse} is set.
...
The \module{axis} has two classes implementing so called anchored
...
and except for some handy axis positions methods:
