...
g = graph.graphxy(height=8, width=8,
                  x=graph.linaxis(min=re_min, max=re_max, title=r'$\Re(c)$'),
                  y=graph.linaxis(min=im_min, max=im_max, title=r'$\Im(c)$'))
g.plot(graph.data(data.data(d), xmin=0, xmax=1, ymin=2, ymax=3, color=4),
       graph.rect(color.palette.Rainbow))
g.dodata() # plot data first, then axes
g.writetofile('mandel.eps')
