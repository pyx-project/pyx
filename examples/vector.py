...
def vector(x1, y1, x2, y2, t, pos=0.5, distance=0.1):
    c = canvas.canvas()
    c.stroke(path.line(x1, y1, x2, y2), canvas.earrow.normal)
    textbox = text.text((1-pos)*x1 + pos*x2, (1-pos)*y1 + pos*y2, t,
                        text.halign.center, text.valign.centerline())
    if distance < 0:
...
c = canvas.canvas()
dx, dy = math.cos(a * math.pi / 180), math.sin(a * math.pi / 180)
x, y = r * dx, r * dy
