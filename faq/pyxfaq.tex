...
% JL: I have mostly checked the FAQ for PyX 0.6. An exception is
% probably the still pending change from graph.data.data to
% graph.data.file

\begin{center}
\LARGE\sffamily Some frequently and\\ 
not so frequently asked questions\\ 
about \PyX\footnote{This version of the FAQ is intended for use with \PyX\ 
version 0.5.1.}\\
\par
{\small\sffamily (version \RCSRevision\ of \RCSDate)}\\[1truecm]
\large
...
The following persons have in one way or the other contributed to the answers
given in this FAQ:\\ 
Jörg Lehmann, Michael Schindler, Andr{\'e} Wobst.
\newpage
...
g.plot(graph.data.data("x.dat", x=1, y=2))\\
g.writeEPSfile("x")
\end{progcode}
...
%See \uaref{q:zeropathattrs} for an explanation of how one can suppress the
%zero lines.
...
\begin{progcode}
g.plot(graph.data.data("datafile.dat"), xmin=0, xmax=1, ymin=2, ymax=3, color=4),\\
...
$y_\mathrm{max}$, and the color value in this order. To determine the color,
we use the \texttt{ReverseRainbow} palette. The last line instructs \PyX{} to
plot the rectangles before plotting the axes. Otherwise, the axes might be
covered partially by the rectangles and, in particular, the ticks might not
be visible. Grey-scale plots can easily be generated by specifying the palette
\texttt{Gray} or \texttt{ReverseGray} (cf.\ appendix C of the manual for a list
...
A frequent task is to rescale data to the interval $[0,1]$. Suppose that the
data are stored in the variable \texttt{data} and that the fifth column 
(denoted in Python by the index 4) has to be rescaled. Then the following 
python code will do the job:
\begin{progcode}
datamax = max(*[d[4] for d in data])\\
datamin = min(*[d[4] for d in data])\\
for d in data:\\
~~d[4] = (d[4]-datamin)/(datamax-datamin)
\end{progcode}
...
\begin{progcode}
g.plot(graph.data.data("datafile.dat"), x=0, y=1, size=2, angle=3),\\
~~~~~~~graph.style.arrow()\\
~~~~~~)
\end{progcode}
...
\question{a}{How do I specify the tick increment? \changed}
{checked for 0.6. Maybe we should add an from pyx.graph import axis}
...
~~~~~~~~~~~~~~~~~~~x=graph.axis.linear(min=1, max=10,\\
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~parter=graph.axis.parter.linear(tickdist=[2,1]))\\
~~~~~~~~~~~~~~~~~~~)
\end{progcode}
}

\question{o}{How do I get rid of the zero line?}
{Obsolete with \PyX{} 0.6. To be deleted.}
...
example:
\begin{progcode}
myaxispainter = graph.axis.painter(zeropathattrs=None)\\
\\
g = graph.graphxy(width=10,\\
~~~~~~~~~~~~~~~~~~x=graph.axis.linear(min=-5, max=5, painter=myaxispainter),\\
...
\begin{progcode}
g.plot(graph.data.data("test.dat"),\\
...
{probably one needs graph.style.symbollie now}
