...
numbered from 1, since the 0th column contain the line number. To
...
and, in particular, the ticks might not be visible. Grey-scale plots
...
draw a zero line by your own. In order to do so you first have fix the
layout information of the graph. Then you can use, for instance, the
\texttt{ygridpath} method of a graph which returns a grid path at the given
axis value. The so obtainted path can easily be stroked on the graph
canvas:
\begin{progcode}
g.finish()\\
